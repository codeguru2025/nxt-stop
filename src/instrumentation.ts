...
// convention). This is the only place the schedulers can safely import Prisma —
...
export async function register() {
  ...
  // (see server.ts's app.prepare().catch(() => process.exit(1))) — non-critical background
  // jobs must never be able to take the whole app down, so failures here just disable them.
  try {
  ...
  try {
    const { startEventReminderScheduler } = await import('./lib/scheduler')
