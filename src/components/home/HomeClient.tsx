...
type Event = {
  ...
  status: string
  lineup?: string
  ticketTypes: { name: string; price: number; soldOut?: boolean; salesChannel?: string }[]
  sellingFast?: boolean
}
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-[#2a2a2a]">
          <span className="text-xs font-semibold text-orange-400">{event.sellingFast && phase !== 'ended' ? 'Selling fast' : ''}</span>
